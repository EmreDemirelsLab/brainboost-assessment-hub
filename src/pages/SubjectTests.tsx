...
export default function SubjectTests() {
  ...
  const fetchTests = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('is_active', true)
        .neq('test_type', 'reading') // Alan testleri için okuma testleri hariç
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Mevcut Alan Testleri</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {filteredTests.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Henüz alan testi bulunmuyor.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
