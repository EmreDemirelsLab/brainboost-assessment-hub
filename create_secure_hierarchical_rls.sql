-- ================================================================
-- GÜVENLİ HİYERAŞİK RLS POLİCİES - TÜM TABLO SİSTEMİ
-- ================================================================
...
-- ============== 3. TEST RESULTS TABLOSU RLS POLİCİES ==============
...
CREATE POLICY "burdon_hierarchy_insert" ON public.burdon_test_results FOR INSERT WITH CHECK (
  can_user_access_user(student_id) AND 
...
-- Stroop Test Results (eğer varsa)
DROP POLICY IF EXISTS "stroop_select_all_authenticated" ON public.stroop_test_results;
DROP POLICY IF EXISTS "stroop_insert_all_authenticated" ON public.stroop_test_results;
DROP POLICY IF EXISTS "stroop_update_all_authenticated" ON public.stroop_test_results;
DROP POLICY IF EXISTS "stroop_delete_all_authenticated" ON public.stroop_test_results;

-- Stroop table varsa policy ekle
DO $$
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'CREATE POLICY "stroop_hierarchy_insert" ON public.stroop_test_results FOR INSERT WITH CHECK (can_user_access_user(student_id) AND conducted_by = get_current_user_id())';
...
-- ============== 6. TEST QUERIES ==============
-- Test için sample sorgular

-- Mevcut kullanıcının rolünü kontrol et
-- SELECT get_current_user_role();

-- Mevcut kullanıcının ID'sini kontrol et
-- SELECT get_current_user_id();

-- Mevcut kullanıcının görebileceği kullanıcıları listele
-- SELECT * FROM users WHERE can_user_access_user(id);

-- Mevcut kullanıcının görebileceği test sonuçlarını listele
-- SELECT * FROM burdon_test_results WHERE can_user_access_user(student_id);

-- ============== SON KONTROL ==============
SELECT 
  'RLS_FUNCTIONS' as table_name, 
